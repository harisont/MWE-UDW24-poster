...
\block{Abstract}{
Ungrammatical text poses significant challenges for off-the-shelf dependency parsers. 
...
\block{Models}{
\begin{itemize}
    \item fine tuning of a Swedish BERT model with the MaChAmp toolkit
    \item experiments with various data configurations:
    \begin{itemize}
        \item corrupted data with normative Swedish to mimic real-world frequency of L2 errors
        \item 50-50 ratio of corrupted and normative sentences
        \item experiments with sequential training
    \end{itemize}
\end{itemize}
...
\begin{minipage}{\linewidth}
    ...
    \resizebox{0.65\columnwidth}{!}{
        ...
        \multicolumn{2}{c|}{\textbf{Name}} & \textbf{\% Normative} & \textbf{\% Errors} \\ \hline
...
\vskip 20pt
\begin{itemize}
    \item synthetic word order errors in training have a small positive effect on actual learner sentences 
    \item larger improvement on (in-domain) corrupted sentences
    \item minimal decline on standard language
\end{itemize}
\vskip 20pt
...
\vskip 20pt
\begin{itemize}
    \item targeted evaluation isolating erroneous segments from SweLL sentences shows a wider performance gap between baseline and specialized models
\end{itemize}
}
